...
$id_reservation_info_devis_client_circuit='';
$stmt = $conn->prepare('SELECT * FROM reservation_info_devis_client_circuit WHERE id_reservation_info_devis_client_circuit !=:id_reservation_info_devis_client_circuit AND devis =:devis ORDER BY id_reservation_info_devis_client_circuit DESC');
$stmt ->bindValue('id_reservation_info_devis_client_circuit', $id_reservation_info_devis_client_circuit);
...
<?php 
        ...
        if(isset($_POST['ajout_devis_client']))
        {
          $date1 = date('Y-m-d'); // Date du jour
            ...
            $stmt5 = $conn ->prepare ("insert into `reservation_info_devis_client_circuit`(`date_creation`)  VALUE (:date_creation)");
            ...
            echo "<meta http-equiv='refresh' content='0;url=ajout_devis_client_circuit.php?id=$id'/>";
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="span9" style="margin-top: 22px;">
                        ...
                        <button class="btn" type="submit" name="ajout_devis_client">Créer un devis direct client</button>
...
?>
                                        <tr>
                                            <td style="vertical-align: middle;"><b><?php echo stripslashes(($account -> id_reservation_info_devis_client_circuit).'/DEVIS-00'.($account -> id_reservation_valeur)); ?></b></td>
...
<?php
    ...
    //$racine="http://localhost/reservation/";
      ...
      if($account -> status==1)
                                        ...
                                        <a href="ajout_devis_client_circuit.php?id=<?php echo md5($account -> id_reservation_info_devis_client_circuit); ?>" class="btn" style="font-size: 10px;padding: 0 14px;margin-bottom: 5px;"><i class="icon-edit"></i> Modifier</a>
                                        <br>
                ...
                $stmt80 = $conn->prepare('SELECT * FROM devis_client_circuit WHERE id_reservation_info_devis_client_circuit =:id_reservation_info_devis_client_circuit');
                $stmt80 ->bindValue('id_reservation_info_devis_client_circuit', $account -> id_reservation_info_devis_client_circuit);
                $stmt80 ->execute();
                $account80 = $stmt80 ->fetch(PDO::FETCH_OBJ);


      if($account -> status==3 OR $account -> status==4)
                                        {

                                            ?>


                                        <a href="<?php echo $racine; ?>interne-devis-account-client-circuit.php?xx=<?php echo MD5($account80 -> id_devis_client_circuit); ?>&xv=<?php echo md5($account80 -> id_reservation_info_devis_client_circuit); ?>" class="btn" style="font-size: 10px;padding: 0 14px;margin-bottom: 5px;"><i class="icon-edit"></i> Acompte</a>
                                        <br>
                                        <a href="<?php echo $racine; ?>interne-devis-coord-circuit.php?xx=<?php echo md5($account80 -> id_reservation_info_devis_client_circuit); ?>" class="btn" style="font-size: 10px;padding: 0 14px;margin-bottom: 5px;"><i class="icon-edit"></i> Coordonées</a>
                                            <?php
