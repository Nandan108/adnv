...
$dossier = str_replace('dossier=', '', $tab[1]);
    ...
    $stmt1 = $conn->prepare('SELECT * FROM prestation_hotel  WHERE id_prestation_hotel= :id_prestation_hotel');
    $stmt1 -> bindValue('id_prestation_hotel', $id_prestation_hotel);
    $stmt1 ->execute();
    $account1 = $stmt1 ->fetch(PDO::FETCH_OBJ);


   $stmt5 = $conn ->prepare ("insert into `prestation_hotel`( `id_hotel`, `id_partenaire`, `debut_validite`, `fin_validite`, `taux_change`, `taux_commission`, `prix_net_adulte`, `prix_brute_adulte`, `total_adulte`, `prix_net_enfant`, `prix_brute_enfant`, `total_enfant`, `prix_net_bebe`, `prix_brute_bebe`, `total_bebe`, `photo`) VALUE (:id_hotel, :id_partenaire, :debut_validite, :fin_validite, :taux_change, :taux_commission, :prix_net_adulte, :prix_brute_adulte, :total_adulte, :prix_net_enfant, :prix_brute_enfant, :total_enfant, :prix_net_bebe, :prix_brute_bebe, :total_bebe,:photo)");
...
echo "<script type='text/javascript'>alert('Duplication prestation hôtel effectué, vous pouvez ajouté un autre');</script>";
