<form id="userSecurityForm" class="form-horizontal" method="post" action="" enctype="multipart/form-data">
    ...
    <?php
    if (isset($_GET['action']) && $_GET['action'] == 'deleteitineraire') {

        $stmt = $conn->prepare('delete from client_itineraire WHERE id_client_itineraire = :id_client_itineraire');
        $stmt->bindValue('id_client_itineraire', $_GET['id_client_itineraire']);
        $stmt->execute();
    ...
    <div style="padding: 30px;">
        ...
        <table style="width: 100%;">
            ...
            $stmt700 = $conn->prepare('SELECT * FROM client_itineraire WHERE id_client =:id_client');
            ...
            while ($account700 = $stmt700->fetch(PDO::FETCH_OBJ)) {
                ...
                <tr>
                    ...
                    <td><a href="https://adnvoyage.com/admin/document/<?php echo $account700->itineraire; ?>"
                            target="_blank"><img src="https://adnvoyage.com/admin/document/pdf.png" width="15" height="20">
                            <?php echo $account700->itineraire; ?>
                        </a></td>
                    <td style="text-align: center"><a
                            href="gerer_client.php?menus=<?= $url ?>&id_client_itineraire=<?php echo $account700->id_client_itineraire; ?>&action=deleteitineraire"
        ...
        <div class="control-group ">
            <label class="control-label">itineraire en PDF</label>
            <div class="controls">
        ...
        <footer id="submit-actions" class="form-actions">
            ...
            <button id="submit-button" type="submit" class="btn btn-primary" name="save_itineraire"
                value="CONFIRM">Enregistrer ce itineraire</button>
        </footer>

        <?php
        if (isset($_POST['save_itineraire'])) {
            ...
            $stmt = $conn->prepare("insert into `client_itineraire`(`itineraire`, `date_ajout`, `id_client_passager`, `id_client`) VALUE (:itineraire,:date_ajout,:id_client_passager,:id_client)");
            $stmt->bindValue('itineraire', $url_photo);
            ...
            echo "<script type='text/javascript'>alert('Ajout de itineraire effectué, vous pouvez ajouter des autres itineraires');</script>";
