<form id="userSecurityForm" class="form-horizontal" method="post" action="" enctype="multipart/form-data">
    ...
    <?php
    if (isset($_GET['action']) && $_GET['action'] == 'deletefacture')
    {
        $stmt = $conn->prepare('delete from client_facture WHERE id_client_facture = :id_client_facture');
        $stmt->bindValue('id_client_facture', $_GET['id_client_facture']);
        $stmt->execute();
    ...
    <div style="padding: 30px;">
        ...
        <table style="width: 100%;">
            ...
            $stmt700 = $conn->prepare('SELECT * FROM client_facture WHERE id_client =:id_client');
            ...
            while ($account700 = $stmt700->fetch(PDO::FETCH_OBJ)) {
                ...
                <tr>
                    ...
                    <td><a href="https://adnvoyage.com/admin/document/<?php echo $account700->facture; ?>"
                            target="_blank"><img src="https://adnvoyage.com/admin/document/pdf.png" width="15" height="20">
                            <?php echo $account700->facture; ?>
                        </a></td>
                    <td style="text-align: center"><a
                            href="gerer_client.php?menus=<?= $url ?>&id_client_facture=<?php echo $account700->id_client_facture; ?>&action=deletefacture"
        ...
        <div class="control-group ">
            <label class="control-label">facture en PDF</label>
            <div class="controls">
        ...
        <footer id="submit-actions" class="form-actions">
            ...
            <button id="submit-button" type="submit" class="btn btn-primary" name="save_facture"
                value="CONFIRM">Enregistrer ce facture</button>
        ...
        if (isset($_POST['save_facture'])) {
            ...
            $stmt = $conn->prepare("insert into `client_facture`(`facture`, `date_ajout`, `id_client_passager`, `id_client`) VALUE (:facture,:date_ajout,:id_client_passager,:id_client)");
            $stmt->bindValue('facture', $url_photo);
            ...
            echo "<script type='text/javascript'>alert('Ajout de facture effectué, vous pouvez ajouter des autres factures');</script>";
