                ...
                <div class="row">
                    <div id="acct-password-row" class="span16">
                        ...
                        <script>
                            ...
                            $stmt70 = $conn->prepare('SELECT * FROM carte_client WHERE lat !=:lat');
                            ...
                            while ($account70 = $stmt70->fetch(PDO::FETCH_OBJ)) {
                                ...
                                if (isset($account70->id_carte_client)) {
                                    if ($account70->site != '') {
                                        $site = "<b><i class='icon-plus'></i>  " . stripslashes(trim($account70->site)) . "</b>";
                                    } else {
                                        $site = '';
                                    }

                                    $description = str_replace("\n", "", stripslashes($account70->description));
                                    ?>

                                    var greenIcon = new LeafIcon({ iconUrl: '<?php echo $account70->photo; ?>' });
                                    L.icon = function (options) {
                                        return new L.Icon(options);
                                    };

                                    L.marker([<?php echo $account70->lat; ?>, <?php echo $account70->longitude; ?>], { icon: greenIcon }).addTo(map)
                                        .bindPopup("<h5><?php echo stripslashes(trim($account70->titre)); ?></h5><img src='<?php echo $account70->photo; ?>' width='150px'><p style='text-align:justify'><?php echo trim($description); ?></p><span style='font-size: 12px;color: #10aefc;'><b><i class='icon-plus'></i>  <?php echo stripslashes(trim($account70->categorie)); ?></b><br><b><i class='icon-plus'></i>  <?php echo stripslashes(trim($account70->recommande)); ?></b><br><b><i class='icon-plus'></i>  <?php echo stripslashes(trim($account70->quartier)); ?></b><?php echo $site; ?></span>");
                            ...
                            $stmt70 = $conn->prepare('SELECT * FROM carte_client');
                            $stmt70->execute();
                            while ($account70 = $stmt70->fetch(PDO::FETCH_OBJ)) {
                                ...
                                if (isset($account70->id_carte_client)) {
                                    ...
                                    var greenIcon2 = new LeafIcon({ iconUrl: '<?php echo $account70->photo; ?>' });
                                    L.icon = function (options) {
                                        return new L.Icon(options);
                                    };


                                    L.marker([<?php echo $account70->lat; ?>, <?php echo $account70->longitude; ?>], { icon: greenIcon2 }).addTo(map)
                                        .bindPopup("<h5><?php echo stripslashes($account70->titre); ?></h5> <img src='<?php echo $account70->photo; ?>' width='150px'><p style='text-align:justify'><?php echo stripslashes(trim($account70->description)); ?></p><b>");
